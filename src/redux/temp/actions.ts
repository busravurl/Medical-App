...
import { SET_MOVIEGENRE, SET_MOVIE, SET_RANDOM_MOVIE, SWITCH_LANGUAGE } from './../actionTypes';
...
const get_random_movie = (page) => {
  return async (dispatch) => {
   try {
    ...
    const data = response.data;
    dispatch({ type: SET_RANDOM_MOVIE, payload: data.data.movies.data[page]});
...
export {get_movie, get_movie_genre,get_random_movie}